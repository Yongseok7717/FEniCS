...
if tau1==1:
 psix1 = Expression('exp(-tn)*sin(x[0]*x[1])*tn*varphi1',varphi1=0.25,  tn=0, degree=5)
else:
 psix1 = Expression('(exp(-tn)-exp(-tn/tau1))*varphi1/(1.0-tau1)*sin(x[0]*x[1])',tau1=0.5, varphi1=0.1,  tn=0, degree=5)


if tau2==1:
 psix2 = Expression('exp(-tn)*sin(x[0]*x[1])*tn*varphi2',varphi2=0.25,  tn=0, degree=5)
else:
...
# Print big tables
print('Tables of error for P1 where k=%d' %k)
print ('V_error for u')
...
if k==1:
    np.savetxt("energy_linear_p1.txt",V_error,fmt="%2.3e")
    np.savetxt("L2w_linear_p1.txt",L2_error,fmt="%2.3e")
    np.savetxt("L2u_linear_p1.txt",Lu_error,fmt="%2.3e")
elif k==2:    
    np.savetxt("energy_quad_p1.txt",V_error,fmt="%2.3e")
    np.savetxt("L2w_quad_p1.txt",L2_error,fmt="%2.3e")
    np.savetxt("L2u_quad_p1.txt",Lu_error,fmt="%2.3e")
...
print('Numeical convergent order of P1 when h=dt for k= %d ' %k)
